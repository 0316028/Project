...
IconSelect.DEFAULT.SELECTED_BOX_PADDING_RIGHT = 20;
IconSelect.DEFAULT.ICONS_WIDTH = 40;
IconSelect.DEFAULT.ICONS_HEIGHT = 40;
IconSelect.DEFAULT.IconText_Width = 300;
IconSelect.DEFAULT.IconText_Height = 40;
IconSelect.DEFAULT.BOX_ICON_SPACE = 10;
IconSelect.DEFAULT.HORIZONTAL_ICON_NUMBER = 5;
IconSelect.DEFAULT.VECTORAL_ICON_NUMBER = 1;

IconSelect.COMPONENT_ICON_FILE_PATH = "https://s26.postimg.org/yf57q4amh/arrow.png";
...
function IconSelect($$elementID, $$parameters) {
    ...
    this.refresh = function($icons){
        ...
        for(var i = 0; i < $icons.length; i++){
            $icons[i].element = _View.createIcon($icons[i].iconFilePath, $icons[i].iconValue, i, $$parameters,$icons[i].IconText);
    ...
    this.getIconName = function(){
      return _icons[_selectedIndex].Name;
    };
    this.getMacAddress = function(){
      return _icons[_selectedIndex].MacAddress;
    };
    ...
    _View.createUI = function($parameters){
      ...
      _View.selectedIconImgElement.setAttribute('src', 'https://s26.postimg.org/mm8bonc21/Icon_Select_Back_Ground.png');
        ...
        selectedIconElement.style.top = ($parameters.selectedBoxPadding).toString() + 'px';
        selectedIconElement.style.left = ($parameters.selectedBoxPadding - PaddingVariable).toString() + 'px';
        ...
        _View.boxScrollElement.style.width = ((($parameters.iconsWidth + 2) * $parameters.vectoralIconNumber) + (($parameters.vectoralIconNumber + 1) *$parameters.boxIconSpace) + $parameters.icontextWidth).toString() + 'px';
        _View.boxScrollElement.style.height = ((($parameters.iconsHeight + 2) * $parameters.horizontalIconNumber) + 
                (($parameters.horizontalIconNumber + 1) * $parameters.boxIconSpace)).toString() + 'px';
    ...
    _View.createIcon = function($iconFilePath, $iconValue, $index, $parameters, $IconText){
        ...
        iconElement.style.width = ($parameters.iconsWidth + $parameters.icontextWidth).toString()+"px";
        iconElement.style.height = ($parameters.iconsHeight).toString()+"px";
        iconElement.style.marginLeft = $parameters.boxIconSpace;
        iconElement.style.marginTop = $parameters.boxIconSpace;
        
        var iconImgElement = document.createElement('img');
		iconImgElement.style.position = "relative";
		...
		/*iconElement.style.width = ($parameters.iconsWidth).toString()+"px";
        iconElement.style.height = ($parameters.iconsHeight).toString()+"px";*/
		
		var icontextElement = document.createElement('div');
		//iconImgElement.setAttribute('text', $IconText);
		icontextElement.setAttribute('class', 'TextClass');
    icontextElement.innerHTML = $IconText;

		icontextElement.style.position = "relative";
		
		icontextElement.style.cssFloat = "left";
		icontextElement.style.lineHeight = ($parameters.iconsHeight/2).toString()+"px";

		//icontextElement.style.display = "inline";
    ...
    _Model.checkParameters = function($parameters){
		...
		$parameters.icontextWidth                 = ($parameters.icontextWidth)                 ? $parameters.icontextWidth               : _default.IconText_Width;
        $parameters.icontextHeight                = ($parameters.icontextHeight)                ? $parameters.icontextHeight              : _default.IconText_Height;
